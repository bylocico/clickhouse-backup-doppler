...
const DOCKER_HUB_TAGS_URL =
	'https://registry.hub.docker.com/v2/repositories/altinity/clickhouse-backup/tags?page_size=100'
...
if (!versions.some((v) => v.version === target.version)) {
	throw new Error(`altinity/clickhouse-backup:${target.version} was not found on Docker Hub`)
}

if (target.version === current) {
	console.log(`Already targeting clickhouse-backup@${current}`)
	process.exit(0)
}
...
console.log(`Updated clickhouse-backup target ${current} -> ${target.version}`)
