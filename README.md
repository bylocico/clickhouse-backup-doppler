...
For example, `ghcr.io/bylocico/clickhouse-backup-doppler:2.7.4` contains `altinity/clickhouse-backup:2.7.4` with the Doppler CLI installed.
...
```bash
docker pull ghcr.io/bylocico/clickhouse-backup-doppler:2.7.4
```
...
docker run \
  ...
  ghcr.io/bylocico/clickhouse-backup-doppler:2.7.4 \
  list
```
...
services:
  clickhouse-backup:
    image: ghcr.io/bylocico/clickhouse-backup-doppler:2.7.4
    environment:
...
```bash
node scripts/update-version.mjs --version 2.7.4
```
