...
For example, `ghcr.io/bylocico/clickhouse-backup-doppler:2.8.0` contains `altinity/clickhouse-backup:2.8.0` with the Doppler CLI installed.
...
```bash
docker pull ghcr.io/bylocico/clickhouse-backup-doppler:2.8.0
```
...
docker run \
  ...
  ghcr.io/bylocico/clickhouse-backup-doppler:2.8.0 \
  list
```
...
services:
  clickhouse-backup:
    image: ghcr.io/bylocico/clickhouse-backup-doppler:2.8.0
    environment:
...
```bash
node scripts/update-version.mjs --version 2.8.0
```
