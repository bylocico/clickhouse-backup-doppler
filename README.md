...
For example, `ghcr.io/bylocico/clickhouse-backup-doppler:2.7.3` contains `altinity/clickhouse-backup:2.7.3` with the Doppler CLI installed.
...
```bash
docker pull ghcr.io/bylocico/clickhouse-backup-doppler:2.7.3
```
...
docker run \
  ...
  ghcr.io/bylocico/clickhouse-backup-doppler:2.7.3 \
  list
```
...
services:
  clickhouse-backup:
    image: ghcr.io/bylocico/clickhouse-backup-doppler:2.7.3
    environment:
...
```bash
node scripts/update-version.mjs --version 2.7.3
```
